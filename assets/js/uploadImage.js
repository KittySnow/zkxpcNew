...
;(function () {
    ...
    function Uploader(options) {
        ...
        settings.id = settings.id || '_temp_' + Math.random().toString().substring(2);
...
// create input, form, iframe
    Uploader.prototype.setup = function () {
        this.form = $(
            '<form id="' + this.settings.id + '" method="post" enctype="multipart/form-data"'
        ...
        this.form.css({
            position: 'absolute',
            top: this.settings.target == 'body' ? $trigger.offset().top : this.settings.top,
            left: this.settings.target == 'body' ? $trigger.offset().left : this.settings.left,
            overflow: 'hidden',
            width: $trigger.outerWidth(),
            height: $trigger.outerHeight(),
            zIndex: findzIndex($trigger) + 10
        }).appendTo(this.settings.target);
        return this;
    };
...
// bind events
    Uploader.prototype.bind = function () {
        ...
        /*$trigger.mouseenter(function () {
         self.form.css({
         top: self.settings.target == 'body' ? $trigger.offset().top : self.settings.top,
         left: self.settings.target == 'body' ? $trigger.offset().left : self.settings.left,
         width: $trigger.outerWidth(),
         height: $trigger.outerHeight()
         });
         });*/
